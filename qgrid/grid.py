...
class QgridWidget(widgets.DOMWidget):
    ...
    def _update_sort(self):
        try:
            ...
            self._df.sort_values(
                self._get_sort_column_name(self._sort_field),
                ascending=self._sort_ascending,
                inplace=True
            )

    def _get_sort_column_name(self, col_name):
        sort_column_name = self._mixed_data_columns.get(col_name)
        if sort_column_name:
            return sort_column_name

        mixed_data_series = \
            self._get_col_series_from_df(col_name, self._df)
        mixed_data_series_unfiltered = \
            self._get_col_series_from_df(col_name, self._unfiltered_df)
        sort_column_name = col_name + self._sort_col_suffix

        self._df[sort_column_name] = mixed_data_series.map(str)
        self._unfiltered_df[sort_column_name] = \
            mixed_data_series_unfiltered.map(str)

        self._mixed_data_columns[col_name] = sort_column_name
        return sort_column_name
                ...
                if col_name in self._sorted_column_cache:
                    ...
                    if len(unique) < 500000:
                        try:
                            ...
                            sort_col_name = \
                                self._get_sort_column_name(col_name)
    ...
    def get_changed_df(self):
        ...
        :rtype: DataFrame
        """
        col_names_to_drop = list(self._mixed_data_columns.values())
